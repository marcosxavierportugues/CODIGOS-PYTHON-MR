...
file_in  = r'D:\00-Obras - SLN\IFMT - SLN\00- ENVIADO IFMT\SEG 2\SEG 2 -PILARES  TERREO-  ENVIADO AO IFMT  16.4.26-V0.xlsx'
file_out = r'D:\00-Obras - SLN\IFMT - SLN\00- ENVIADO IFMT\SEG 2\SEG 2 -PILARES TERREO- ORGANIZADO-BITOLA - 16.4.26-V0.xlsx'

df = pd.read_excel(file_in, sheet_name='Sheet1', header=0)
df.columns = [str(c) for c in df.columns]

# Item pairs detected: (desc_col, qty_col) 0-based
item_pairs = [(58,59),(60,61),(62,63),(64,65),(66,67),(68,69),(70,71),(72,73),(74,75),(76,77),(78,79)]

# Key columns
COL_NAME  = 0   # Pilar name
COL_LAYER = 7   # Item Layer
...
for _, row in df.iterrows():
    ...
    rec = {'PILAR': pilar, 'CAMADA': layer, 'CONCRETO_m3': concreto, 'FORMA_m2': forma}
    for b in bitolas:
...
print(f'Total pilares: {len(result)}')
...
header_labels = ['PILAR', 'CAMADA', 'CONCRETO', 'FORMA'] + [f'ACO O{b}mm' for b in bitolas]
...
for ri, rec in enumerate(records, 3):
    ...
    vals = [rec['PILAR'], rec['CAMADA'], rec['CONCRETO_m3'], rec['FORMA_m2']] + [rec[f'ACO_{b}mm_kg'] for b in bitolas]
...
col_widths = [12, 20, 14, 14] + [13] * len(bitolas)
...
ws2['A1'] = 'RESUMO GERAL - SEG 2 PILARES TERREO'
...
ws3['A1'] = 'RESUMO POR CAMADA - SEG 2 PILARES TERREO'
