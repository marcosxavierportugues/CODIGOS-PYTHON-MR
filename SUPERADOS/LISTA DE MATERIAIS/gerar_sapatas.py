"""
Gerador de planilha de sapatas a partir de exportação Navisworks/AltoQi Eberick.
...
# ── CONFIG ──────────────────────────────────────────────────────────────────
FILE_IN  = Path(r"D:\00-Obras - SLN\IFMT - SLN\0000- NAVISWORKS IFMT\IFMT - ENVIADO - TODAS AS SAPTAS.xlsx")
...
def load_source(path: Path, sheet) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        rec = {'SAPATA': str(row[col_name]).strip(),
        ...
        for dc, qc in zip(desc_cols, qty_cols):
            ...
            if 'Concreto' in desc:
                ...
                rec['FORMA_m2'] += qty
            elif 'Armadura' in desc or 'Aço' in desc or 'Aco' in desc or 'Armadura' in desc.encode('latin-1', errors='replace').decode('latin-1', errors='replace'):
...
def build_prorrata(wb, result, bitolas):
    ...
    col_map.append(('SAPATA',  '',   hdr_blue,   False, 'SAPATA',       None))
    ...
    for ri, (_, rec) in enumerate(result.iterrows(), 4):
        ...
        for ci, (_, _, fill, is_pct, key, tot) in enumerate(col_map, 1):
            if key == 'SAPATA':
                set_cell(ws4, ri, ci, rec['SAPATA'], fill=rf, align=left_al)
            elif key == 'CAMADA':
...
def main():
    ...
    print(f"  {len(result)} sapatas | bitolas ativas: {bitolas}")
