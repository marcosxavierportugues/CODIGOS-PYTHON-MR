...
def _confirm_cancel():
    ...
    result = [False]
    def yes(): result[0] = True;  win.destroy(); r.destroy()
    def no():  result[0] = False; win.destroy(); r.destroy()
    frame = tk.Frame(win); frame.pack(pady=8)
...
def _ask_main_action():
    ...
    for text, value in [("Gerar lista de material", 1), ("Unir listas de materiais", 2), ("Cancelar", 3)]:
        tk.Radiobutton(win, text=text, variable=action, value=value,
                       font=("Arial", 11), anchor="w").pack(fill="x", padx=20, pady=2)
    result = [3]
    def ok():     result[0] = action.get(); win.destroy(); r.destroy()
    def cancel(): result[0] = 3;            win.destroy(); r.destroy()
...
def _ask_choice(title, prompt, options):
    ...
    win = tk.Toplevel(r)
    win.title(title); win.attributes('-topmost', True)
    ...
    def cancel():
        if _confirm_cancel(): raise SystemExit("Cancelado pelo usuário")
...
def _find_column(cols, keywords, exclude=None):
    # FIX: normalize exclude terms so accented variants match normalized column names
    norm_exclude = [_normalize_text(ex) for ex in (exclude or [])]
    for c in cols:
        norm = _normalize_text(c)
        if any(ex in norm for ex in norm_exclude):
            continue
...
def _build_merge_sheet(wb, title, df, bitolas, meta=None):
    ...
    hrow = _merge_header_row(ws, meta, title, len(headers))
    for ci, h in enumerate(headers, 1):
        _h(ws, hrow, ci, h, fl=H_PILAR)
    for ri, row in enumerate(df.to_dict(orient="records"), start=hrow + 1):
        for ci, field in enumerate(field_names, 1):
            ...
            _c(ws, ri, ci, value, fl=ALT[(ri - (hrow + 1)) % 2], al=R if fmt else L, fmt=fmt)
...
def _build_merge_summary_sheet(wb, summaries, bitolas, meta=None):
    ...
    hrow = _merge_header_row(ws, meta, "RESUMO POR SEGMENTO", len(headers))
    element_label = meta["elemento"] if meta and meta.get("elemento") else summaries[0]["group_label"]
    for ci, h in enumerate(headers, 1):
        _h(ws, hrow, ci, h, fl=H_PILAR)
    for ri, summary in enumerate(summaries, start=hrow + 1):
        values = [summary["segment"], element_label,
                  summary["totals"]["CONCRETO_m3"], summary["totals"]["FORMA_m2"]]
    ...
    ri = hrow + len(summaries) + 1
    _c(ws, ri, 1, "TOTAL GERAL",  fnt=WF, fl=H_TOTAL, al=L)
    _c(ws, ri, 2, element_label,  fnt=WF, fl=H_TOTAL, al=L)
...
def _build_merge_total_sheet(wb, total, element_label, bitolas, meta=None):
    ...
    hrow = _merge_header_row(ws, meta, f"RESUMO TOTAL {element_label}", len(headers))
    for ci, h in enumerate(headers, 1):
        _h(ws, hrow, ci, h, fl=H_PILAR)
    values = [element_label, total["CONCRETO_m3"], total["FORMA_m2"]] + \
             [total.get(f"ACO_{b}mm_kg", 0) for b in bitolas]
    for ci, value in enumerate(values, 1):
        fmt = "#,##0.000" if ci >= 2 else None
        _c(ws, hrow + 1, ci, value, fnt=WF, fl=H_TOTAL, al=R if fmt else L, fmt=fmt)
...
def _analyze_merge_files(paths):
    ...
    for idx, path in enumerate(paths, start=1):
        ...
        for b in bitolas:
            totals[f"ACO_{b}mm_kg"] = df[f"ACO_{b}mm_kg"].sum() if f"ACO_{b}mm_kg" in df.columns else 0
        summaries.append({
            "path": path, "df": df, "bitolas": bitolas,
            "categories": categories, "levels": levels, "segment": segment,
...
def merge_material_lists():
    ...
    group_label = meta["elemento"] or summaries[0]["group_label"]
    for s in summaries:
        df = s["df"].copy()
        for b in all_bitolas:
            ...
            if col not in df.columns:
                df[col] = 0.0
        _build_merge_sheet(wb, f"TODOS - {group_label} {s['segment']}", df, all_bitolas, meta)
    _build_merge_summary_sheet(wb, summaries, all_bitolas, meta)
    ...
    _build_merge_total_sheet(wb, total, group_label, all_bitolas, meta)
    wb.save(file_out)
    _copy_cache(file_out)
    if _ask_yes_no("Planilha gerada", f"Planilha gerada:\n{file_out.name}\n\nDeseja abrir agora?"):
        try: os.startfile(file_out)
        except Exception as e: print(f"  (erro ao abrir: {e})")
    print(f"\nSalvo em:\n{file_out}")
...
def coletar_metadados_uniao():
    ...
    def _ev(env, title, prompt, default=""):
        v = os.environ.get(env, "").strip().upper()
        return v if v else (_ask_str(title, prompt, default) or default).strip().upper()

    elemento  = _ev("LM_ELEMENTO",  "Elemento",  "Qual ELEMENTO?\n(ex: PILARES, VIGAS, SAPATAS)", "ELEMENTO")
    pavimento = _ev("LM_PAVIMENTO", "Pavimento", "Qual PAVIMENTO?\n(ex: PAV BALDRAMES, TERREO)",  "PAV")
    obra      = _ev("LM_OBRA",      "Obra",      "Qual OBRA?\n(ex: IFMTR, IFMTC, OBRA XYZ)",     "IFMT")
    ...
    print(f"  Elemento: {elemento} | Pavimento: {pavimento} | Obra: {obra} | Tipo: {tipo} | Data: {data_exp}")
...
def coletar_metadados():
    ...
    pavimento = _ev("LM_PAVIMENTO", "Pavimento", "Qual PAVIMENTO?\n(ex: PAV BALDRAMES, TERREO, 1 PAV)",   "PAV")
    segmento  = _ev("LM_SEGMENTO",  "Segmento",  "Qual SEGMENTO?\n(ex: SEG 1, SEG 2, SEG 3, TODOS)",     "TODOS")
    obra      = _ev("LM_OBRA",      "Obra",      "Qual OBRA?\n(ex: IFMTR, IFMTC, OBRA XYZ)",             "IFMT")
    tipo_env  = os.environ.get("LM_TIPO", "").strip().upper()
    tipo      = tipo_env if tipo_env in ("EXECUTIVO", "OFICIAL") else \
                _ask_choice("Tipo", "Projeto EXECUTIVO ou OFICIAL?", ["EXECUTIVO", "OFICIAL"])
    ...
    if modo_env in ("A", "RESUMO"):
        ...
        modo_resumo = "(A)" in _ask_choice(
            "Modo de saida", "O que voce quer gerar?",
            ["(A) So RESUMO (totais por material)", "(B) LISTA COMPLETA (todos os itens + resumo)"])
    obs_env    = os.environ.get("LM_OBS", "").strip().upper()
    observacao = obs_env if obs_env else \
                 _ask_str("Observacao",
    ...
    print(f"  Elemento: {elemento} | Pav: {pavimento} | Seg: {segmento} | Obra: {obra}")
    print(f"  Tipo: {tipo} | Data: {data_exp} | Obs: {observacao or '(nenhuma)'}")
...
def coletar_arquivos(meta):
    ...
    seg_files = {}
    if _ask_yes_no("Segmentos", "Voce tem arquivos separados por segmento?\n(SEG 1, SEG 2, etc.)"):
        i = 1
        while _ask_yes_no("Segmentos", f"Adicionar arquivo SEG {i}?"):
            f = _ask_file(f"Planilha SEG {i}")
            seg_files[f"SEG {i}"] = f
            print(f"   SEG {i}: {f.name}")
            i += 1
    file_out = _ask_save("Salvar planilha como...",
                         meta['nome_base'].replace("/", "-").replace("°", "") + ".xlsx")
...
def _norm_cols(df):
    ...
    for c in df.columns:
        nc = str(c)
        nc = re.sub(r"x000D_", "", nc)
        nc = re.sub(r"[\r\n]+", " ", nc)
        nc = re.sub(r"Forma\s*-\s*Estrutura\s*-\s*Concreto", "Forma", nc, flags=re.IGNORECASE)
        nc = re.sub(r"\b(?:m³|m3|m2|kg)\b", "", nc, flags=re.IGNORECASE)
...
# Exclude list shared by both load_qto paths
_ELEM_EXCLUDE = [
...
def _populate_elem(df, elem_cols):
    """Creates/fills ELEM column from elem_cols; drops originals; returns updated df."""
    if "ELEM" not in df.columns:
        df = df.copy()
        df["ELEM"] = ""
    for c in elem_cols:
        if c != "ELEM":
            df["ELEM"] = df["ELEM"].where(
                df["ELEM"].astype(str).str.strip() != "",
...
def load_qto(path, default_nivel=None):
    ...
    # Detectar formato AltoQi bruto (pares Descricao/Quantidade)
    has_raw_qto_cols = bool(
        [c for c in cols if "Descri" in c] and
        [c for c in cols if "Quantidade" in c or "Quantity" in c])

    # ── Caminho 1: arquivo ja processado (sem pares Descricao/Quantidade) ────
    elem_cols = _find_columns(cols, ["elem", "elemento", "nome"], exclude=_ELEM_EXCLUDE)
    if elem_cols:
        df = _populate_elem(df, elem_cols)
        cols       = [str(c).strip() for c in df.columns]
        cols_upper = [c.upper() for c in cols]

    if not has_raw_qto_cols and ("ELEM" in cols_upper or "ELEMENTO" in cols_upper or elem_cols):
        ...
        if not aco_cols:
            rename = {c: f"ACO_{re.match(r'O([\d.]+)mm', str(c)).group(1)}mm_kg"
                      for c in df.columns if re.match(r"O([\d.]+)mm", str(c))}
            if rename:
                df       = df.rename(columns=rename)
                aco_cols = [c for c in df.columns if re.match(r"ACO_[\d.]+mm_kg", str(c))]
        for col in ["CONCRETO_m3", "FORMA_m2"] + aco_cols:
    ...
    # ── Caminho 2: AltoQi Eberick bruto (pares Descricao/Quantidade) ─────────
    cols = list(df.columns)

    elem_cols = _find_columns(cols, ["name", "nome", "elemento", "elem"],
                              exclude=_ELEM_EXCLUDE + ["layer"])
    if elem_cols:
        df        = _populate_elem(df, elem_cols)
        col_name  = "ELEM"
    else:
    ...
    if col_name in df.columns and df.columns[0] != col_name:
        ordered = [col_name] + [c for c in df.columns if c != col_name]
        df      = df[ordered]
    ...
    for _, row in df.iterrows():
        # FIX: pular linhas com nome de elemento vazio ou NaN
        elem_val = str(row[col_name]).strip()
        if not elem_val or elem_val.lower() == "nan":
            continue
        nivel_raw = row[col_layer] if col_layer else (default_nivel or "UNICO")
        rec = {"ELEM": elem_val, "NIVEL": normalizar_nivel(nivel_raw),
...
def load_segmentos(seg_files):
    ...
    for seg_label, f in seg_files.items():
        df   = _norm_cols(pd.read_excel(f, sheet_name=0)).dropna(axis=1, how="all")
        cols = [str(c).strip() for c in df.columns]
        elem_cols = _find_columns(cols, ["name", "nome", "elemento", "elem"],
                                  exclude=_ELEM_EXCLUDE + ["layer"])
        if elem_cols:
        ...
        frames.append(tmp)
    return pd.concat(frames, ignore_index=True).drop_duplicates("ELEM")
...
BRD = Border(left=Side(style="thin", color="BFBFBF"),
...
def write_total_row(ws, ri, label, sub, bitolas, sfill, n_extra):
    ...
    # FIX: guard against KeyError when columns are absent in already-processed files
    conc  = sub["CONCRETO_m3"].sum() if "CONCRETO_m3" in sub.columns else 0
    forma = sub["FORMA_m2"].sum()    if "FORMA_m2"    in sub.columns else 0
    _c(ws, ri, ci, conc,  fnt=WF, fl=sfill, al=R, fmt="#,##0.000"); ci += 1
    _c(ws, ri, ci, forma, fnt=WF, fl=sfill, al=R, fmt="#,##0.00");  ci += 1
    for b in bitolas:
        col = f"ACO_{b}mm_kg"
...
def build_aba(wb, titulo_aba, titulo_header, sub, bitolas,
    ...
    # FIX: initialize extra_cols before branches so the variable is always defined
    ...
    for ci, w in enumerate([col[1] for col in extra_cols] + [13, 13] + [12] * len(bitolas), 1):
        ws.column_dimensions[get_column_letter(ci)].width = w
...
def build_total_geral(wb, data, bitolas, meta, grupos_label=None, grupo_col=None):
    ...
    titulo = (f"{titulo_prefix} - {meta['elemento']} - {meta['pavimento']} - "
              f"{meta['obra']} - {meta['tipo']} - {meta['segmento']} - {meta['data']}{obs_parte}")
    ...
    for idx, (cat, desc, col, un) in enumerate(rows_def, 1):
        ...
        if grupos_label and grupo_col and grupo_col in data.columns:
            seg_vals = [data[data[grupo_col] == gl][col_ok].sum() if col_ok else 0
                        for gl in grupos_label]
        else:
    ...
    # FIX: TOTAL ACO — calcular por segmento corretamente (antes era sempre 0)
    ...
    for ci_seg, gl in enumerate(seg_labels, start=5):
        if grupos_label and grupo_col and grupo_col in data.columns:
            seg_aco = sum(data[data[grupo_col] == gl][c].sum()
                         for c in aco_cols if c in data.columns)
        else:
            seg_aco = sum(data[c].sum() for c in aco_cols if c in data.columns)
        _c(ws, ri, ci_seg, seg_aco, fnt=WF, fl=H_TOTAL, al=R, fmt="#,##0.000")
...
def main():
    ...
    if has_layer_col:
        ...
        if not (len(niveis_do_arquivo) == 1 and niveis_do_arquivo[0] == meta["pavimento"]):
            opts = [
                f"Usar Layer do arquivo ({', '.join(str(n) for n in niveis_do_arquivo[:4])}"
                f"{'...' if len(niveis_do_arquivo) > 4 else ''})",
    ...
    segmentos_unicos = sorted(s for s in data["SEGMENTO"].unique() if s != "SEM SEGMENTO")
    ...
    if multi_seg and multi_niv:
        for si, seg in enumerate(segmentos_unicos):
            for niv in niveis_unicos:
                ...
                build_aba(wb, f"{seg} - {niv}"[:31], "ELEMENTO", sub, bitolas,
        ...
        for si, seg in enumerate(segmentos_unicos):
            ...
            build_aba(wb, format_aba_nome(seg[:31], meta), "ELEMENTO", sub, bitolas,
                      meta["modo_resumo"], S_FILLS[si % len(S_FILLS)],
        ...
        for niv in niveis_unicos:
            ...
            build_aba(wb, niv[:31], "ELEMENTO", sub, bitolas, meta["modo_resumo"], S_FILLS[0])

    else:
        build_aba(wb, format_aba_nome("TODOS", meta), "ELEMENTO", data, bitolas,
                  meta["modo_resumo"], H_TOTAL)

    if tem_sem_seg:
        sub_sem = data[data["SEGMENTO"] == "SEM SEGMENTO"].reset_index(drop=True)
        build_aba(wb, "SEM SEGMENTO", "ELEMENTO", sub_sem, bitolas, meta["modo_resumo"], H_SEM)
    ...
    print(f"\nSalvo em:\n{file_out}")
    print(f"Total: {len(data)} elementos | Bitolas: {bitolas}")
