...
def _build_merge_summary_sheet(wb, summaries, bitolas, meta=None):
    ...
    for ri, summary in enumerate(summaries, start=header_row + 1):
        values = [summary["segment"], element_label, summary["totals"]["CONCRETO_m3"], summary["totals"]["FORMA_m2"]]
...
def merge_material_lists():
    ...
    for s in summaries:
        ...
        _build_merge_sheet(wb, title, df, all_bitolas, meta)
...
def _bitola_from_aco_column(column):
    m = re.search(r"ACO_([\d.]+)mm", str(column), flags=re.IGNORECASE)
    return float(m.group(1)) if m else None


def _find_description_columns(cols):
    return sorted(
        (c for c in cols if "descri" in _normalize_text(c)),
        key=lambda c: _normalize_text(c),
    )


def _find_quantity_columns(cols):
    return sorted(
...
def load_qto(path, default_nivel=None):
    ...
    raw_desc_cols = _find_description_columns(cols)
    raw_qty_cols = _find_quantity_columns(cols)
    ...
    if ("ELEM" in cols_upper or "ELEMENTO" in cols_upper or elem_cols) and not has_raw_qto_cols:
        ...
        aco_cols = _find_aco_columns(df.columns)
        if not aco_cols:
            rename = {}
            for c in df.columns:
    ...
    desc_cols = _find_description_columns(cols)
    qty_cols  = _find_quantity_columns(cols)
    ...
    aco_cols = sorted(
        _find_aco_columns(result.columns),
...
def build_total_geral(wb, data, bitolas, meta, grupos_label=None, grupo_col=None):
    ...
    titulo = f"{titulo_prefix} - {meta['elemento']} - {meta['pavimento']} - {meta['obra']} - {meta['tipo']} - {meta['segmento']} - {meta['data']}{obs_parte}"
